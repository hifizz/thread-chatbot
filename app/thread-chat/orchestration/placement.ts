...
export const replaceStrategy: PlacementStrategy = (slots, threadId, ctx) => {
  ...
  const replaceAt = (idx: number): PlaceResult => {
    ...
    return {
      slots: next,
      effect: { kind: "replaced", idx, replacedId, prevSlots },
    }
  }
  ...
  if (slots.length < ctx.maxExpanded) {
    ...
    return {
      slots: insertSlot(slots, insertAt, threadId),
      effect: { kind: "appended" },
    }
  }
...
export const foldStrategy: PlacementStrategy = (slots, threadId, ctx) => {
  ...
  if (at >= 0) {
    ...
    const insertAt = ctx.hint?.keepSource
      ? sourceIdxOf(slots, ctx) + 1
      : slots.length
    next = insertSlot(slots, insertAt, threadId)
    appended = true
  }

  const doneEffect: PlaceEffect = appended
    ? { kind: "appended" }
    : { kind: "visible" }
  ...
  if (expanded.length <= ctx.maxExpanded)
    return { slots: next, effect: doneEffect }
  ...
  //   （细条仍在场，一键可原地展开）。
  const preferred = expanded.filter(
    (s) => s.id !== threadId && s.id !== ctx.sourceId
  )
  const pool = preferred.length
    ? preferred
    : expanded.filter((s) => s.id !== threadId)
...
export function trimSlots(slots: Slot[], maxExpanded: number): Slot[] {
  ...
  while (out.length && expandedOf(out).length > Math.max(0, maxExpanded))
    out.shift()
  return out
}
...
export function normalizeForReplace(
  ...
  return {
    slots: expanded.slice(dropCount),
    dropped: expanded.slice(0, dropCount).map((s) => s.id),
  }
}
