...
export function useColumnResize({
  ...
  const colEl = (id: string): HTMLElement | null =>
    colsRef.current?.querySelector<HTMLElement>(
      `.column[data-thread-id="${CSS.escape(id)}"]`
    ) ?? null

  /** 行内全部展开列（DOM 顺序；细条不参与宽度体系） */
  const rowCols = (): { el: HTMLElement; id: string }[] => {
    const els = colsRef.current?.querySelectorAll<HTMLElement>(
      ".column[data-thread-id]"
    )
    if (!els) return []
  ...
  const measureRow = (): Record<string, number> => {
    ...
    for (const { el, id } of rowCols())
      patch[id] = el.getBoundingClientRect().width
    return patch
  }
  ...
  const finishDrag = (s: DragSession) => {
    ...
    if (s.handle.hasPointerCapture(s.pointerId))
