...
export function CodeFontPicker() {
  ...
  return (
    <>
      ...
      <NativeSelect id="code-font-select" value={override ? CUSTOM_GOOGLE_FONT_OPTION : selected} onChange={(event) => selectFont(event.target.value)}>
      ...
      <small role="status">
        {override ? "Google 字体 · 可在下方管理" : failed ? "加载失败，请切换字体后重试" : loading ? "正在加载字体…" : "system" in font ? "系统字体 · 未安装时使用等宽回退" : "已应用于代码块"}
      </small>
    </>
  )
}
