...
export default function HarnessLayout({ children }: { children: React.ReactNode }) {
  return <ModelCatalogProvider refreshEnabled={false} initial={{ defaultModelId: DEFAULT_MODEL_ID, models: THREAD_CHAT_MODEL_OPTIONS.map((m) => ({ ...m, logo: "auto", contextWindow: null })) }}>{children}</ModelCatalogProvider>
}
