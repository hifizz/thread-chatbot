...
/* beautiful-ui 官方 gallery「02 Thinking」卡片的单卡复刻；
 * 交互外壳（复制/查看代码/变体切换）与官方 components/site/Grid.tsx 一致。 */

const ENTRY = {
  id: "thinking-state",
  title: "Thinking",
  caption: "Expandable traces — steps, reasoning, search, coding.",
  file: "primitives/ThinkingState.tsx",
  variants: ["Steps", "Reasoning", "Search", "Coding"],
};

const REGISTRY_URL = `https://www.beautifului.dev/r/${ENTRY.id}.json`;
const GLOBALS_URL =
...
export function ThinkingDemo({ code }: { code: string }) {
  const [open, setOpen] = useState(false);

  return (
    <>
      <div className="flex w-full flex-col pt-8 pb-12 lg:pt-10 lg:pb-16">
        <ThinkingCard code={code} onOpen={() => setOpen(true)} index={1} />
      </div>
      {open && <Overlay code={code} onClose={() => setOpen(false)} />}
    </>
  );
}
...
function Overlay({ code, onClose }: { code: string; onClose: () => void }) {
  const { copied, copy } = useCopy(code);
        ...
        <div className="primitive-card-bar flex items-center justify-between gap-3 border-b border-line">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11.5px] text-ink-3">
              Self-contained — needs only the foundation tokens.
            </p>
            <p
              className="mt-1.5 truncate font-mono text-[11px] text-ink-3"
              title={REGISTRY_URL}
            >
              <span className="text-ink-2">$</span> npx shadcn add {REGISTRY_URL}
            </p>
          </div>
