...
输入框 SHALL 保留原有阅读风格，支持显式 @ 按钮、中文前缀触发、按标题/类型/来源 Thread 搜索、首项以可见 outline ring 高亮并支持上下键与 Enter 选择；键盘焦点保留在输入框。引用标题不得被当作普通文字编辑。
...
#### Scenario: 冻结胶囊的光标边界
- **WHEN** 用户按 Enter 插入引用、通过左右键跨越胶囊或点击胶囊后继续输入
- **THEN** 插入和点击后光标位于胶囊右侧，左右键只在胶囊两侧移动，不进入标题内部；输入空格和文字不得替换引用，显式删除仍作为整体处理
