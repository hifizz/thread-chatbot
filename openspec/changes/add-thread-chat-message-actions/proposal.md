...
- 将 generation 真相与消息结构一起协调为明确的轮次恢复状态：运行中继续显示后台生成，终态合并结果，空 assistant 无 generation 时转为可重试错误，末尾 user 无 assistant/generation 时标记为可恢复孤儿轮次。
...
- 点赞/点踩绑定具体 generation，服务端持久化并支持覆盖选择，刷新后恢复；重新生成的新答案不会继承旧 generation 的反馈。
...
- `thread-chat-message-actions`: thread-chat 消息操作、孤儿轮次恢复、最后一轮编辑后重新生成、generation 级反馈及跨列/画布一致交互契约。
...
- 服务端 API：树 GET 增加 recoverable turn 摘要；generation 查询/反馈接口增加 generation-scoped feedback；`/api/chat` 明确验证编辑后重新生成与孤儿轮次新建 assistant 的合法 turn。
- 数据库：为 branch generation 增加可空反馈值与更新时间；为 `branch_trees` 增加单调 revision，整树 PUT、生成事务和 active-leaf 切换使用 CAS，避免旧标签页覆盖消息图或版本选择。
- 测试：覆盖空占位、孤儿 user、运行中/终态 generation、刷新恢复、复制状态、编辑发送、Retry attempt、消息版本切换、旧版本派生分栏和 Artifact 可达性、反馈持久化、越权访问，以及列/画布一致性。
