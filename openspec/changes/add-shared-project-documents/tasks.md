...
- [x] 10.1 会话写命令按 Project 排他锁 → Thread → Message 获取锁，移除全局死锁重试；原生双连接确认竞争发生在 Project 锁入口。
- [x] 10.2 项目运行时为唯一目录刷新入口，生成结束/打开面板仅通知刷新；回归覆盖迟到响应丢弃、通知合并与释放后不写入。
- [x] 10.3 Bootstrap 和轮询复用一次 SQL 的当前目录；打开 Thread 才读取历史元数据，固定正文按需加载。原生并发提交前后目录完整；EGO 验证桌面/手机历史阅读、当前列表与单一 @ 候选。

本节仅覆盖这三项审查修复，不替代上方仍未完成的真实模型全流程、系统分享和发布验收。

## 11. 四次审查：收据与生产代码边界

- [x] 11.1 清退旧全文清单生产 SQL 到测试夹具，移除未交付的文档级归档预留；保留规范要求的按 Document 当前读取契约。
- [x] 11.2 新收据增加 kind 判别，读取兼容无标签 v1，未知标签/版本不推进通知；原生数据库覆盖新旧格式。
- [x] 11.3 旧清单按固定身份和提交集合匹配，覆盖 JSON 键序/数组顺序变化、身份不符拒绝和普通请求不扫描收据。

本节是自动专项回归；本轮未重跑真实模型或浏览器，未完成的发布门槛保持原状态。

## 12. 工具常驻与完成事件刷新

- [x] 12.1 保留文档工具常驻，完整策略只放 system，工具使用短描述。
- [x] 12.2 在 schema/CLAUDE.md 明确恢复入口及末尾兜底的顺序限制；回归原位替换、缺失结果追加和重复恢复。
- [x] 12.3 删除目录定时轮询、启动后的重复拉取及面板刷新入口；Bootstrap 初始加载，本地生成终态触发刷新，接受跨浏览器目录不自动同步。

本轮通过类型检查、文档/客户端/提示词缓存专项回归；没有重跑真实模型或浏览器全流程。
