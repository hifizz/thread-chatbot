...
- [x] 5.1 接受主线新用户消息时获取一致版本清单，保存服务端生成的固定上下文 Part；范围选择可见，首次文档/未接收提交都有明确记录。
- [x] 5.2 编译固定 Revision 全文和提交说明，按实际完整 Artifact 去重，不按 Document ID 跳过新版，不重新解释旧 @artifact 或修改 forkContext。
- [x] 5.3 保存本轮文档清单的实际请求使用收据，区分计划输入与已发出的请求；预算/生成前失败不推进接收位置，打开面板不代表模型接收。
- [x] 5.4 覆盖重试/重生复用旧清单、执行期间出现新版本、显式 read 新版另留结果、子 Thread 不自动加载无关进展及 Quote 删除不恢复。
- [ ] 5.5 集成最终上下文预算，验证过多文档明确失败并保留未接收进展，可通过可见范围选择重新提交，unknown 计量不伪称通过。
...
- [x] 6.4 实现 ProjectDocumentUpdates 主线进展及本轮范围选择，UI 展示与模型使用状态分开，不伪造已读游标。
...
- [ ] 7.2 核对最终模型请求及工具执行序列：最新读取、readId、重试预算、删除目标、语义前提变化、两处修改一次提交、主线清单与上下文预算。
- [ ] 7.3 桌面与手机浏览器完成 F1 主线创建→A/B 更新冲突→查看历史/差异→主线接收→旧引用/分享回放流程，并保存验收截图与失败恢复证据。
