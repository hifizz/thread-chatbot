...
### Requirement: 文档目录与正文分开加载

项目初始化和后台目录同步 SHALL 只返回 Artifact 元数据。当前文档条目 SHALL 由服务端 Document 目录决定；目录同步生命周期 SHALL 属于项目运行时，不依赖阅读面板打开。固定正文 SHALL 按 Artifact ID 按需读取并独立缓存，来源回复状态更新不得替换已读正文、选区或草稿。

#### Scenario: 来源回复完成后恢复引用资格
- **WHEN** 文档条目首次载入时来源回复仍在生成，随后来源回复完成
- **THEN** 后台目录同步更新来源状态，新建引用候选按完成状态恢复；无需重新加载已缓存全文

#### Scenario: 打开历史正文
- **WHEN** 用户从历史消息卡片打开旧版本
- **THEN** 系统读取对应固定 Artifact 全文，保留旧正文；目录中的当前版本不因此改变
