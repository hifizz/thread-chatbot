...
本 Change 接续 #143 的 `add-markdown-artifact-forks`，定义“并行讨论 → 读取最新文档 → 提交局部修改 → 保存历史 → 主线接收进展”。本 PR 现已进入实施，完成状态与本地验收门槛见 implementation.md。
...
- 主线展示已提交的文档进展，下一条用户消息接收固定的版本清单和变更说明；记录实际模型使用的版本，历史消息及重试不动态读取 head。
- 定义版本阅读、差异查看、工具执行状态、冲突反馈、主线进展组件，以及 DTO、模块和验证任务。
...
- `document-update-context`: 主线接收进展、模型输入版本固定、当前/历史阅读、操作反馈及与 Artifact 分支的衔接。
...
- 客户端：扩展 ProjectPanel/ArtifactDetail，增加文档版本与变更视图、工具状态卡片、主线进展入口；复用现有 Markdown、差异库和组件。
