...
项目当前目录由 Bootstrap 初次加载，之后仅在本浏览器观察到生成终态（包括刷新页面后恢复跟踪的生成终态）时通知运行时拉取。没有定时轮询，也不因打开面板或窗口聚焦而刷新。运行时合并通知并丢弃失效请求，目录更新不改变所选固定版本、选区或草稿。其他浏览器产生的更新可以保持不可见，用户刷新页面后重新加载；失败显示刷新错误，不启动自动重试循环。历史卡片元数据与固定正文仍按阅读需求加载。
...
文档工具是核心能力，不按项目文档数量切换注册集合，避免由空目录变为非空目录造成文档工具与策略前缀变化；这不代表其他生成模式或内容变化不会影响缓存。完整文档策略只放 system 一次，更新工具使用短描述。关闭写入开关仍移除写工具。

持久化消息通过 restoreDocumentToolParts 恢复工具结果。已匹配调用原位替换，缺失调用位置时追加是保留提交事实的兜底，不保证完整时序；本期不引入顺序字段或事件重建系统。
