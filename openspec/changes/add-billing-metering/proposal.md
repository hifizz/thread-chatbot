...
- 新增账户汇总 API `app/api/billing/summary/route.ts`（余额 + 按 threadId 可选的用量/费用聚合 + 最近一次调用）与对账定时任务 `app/api/billing/reconcile/route.ts`（`CRON_SECRET` 鉴权），`vercel.json` 配置每日触发一次 cron（受 Hobby 套餐 cron 每天限一次约束）。
...
- **部署**：`vercel.json` 新增 cron 配置（每日 `0 3 * * *` 触发对账；Hobby 套餐 cron 每天限一次）；`CRON_SECRET` 环境变量为对账端点的必需前置。
