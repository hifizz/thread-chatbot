...
- `artifact-reference-context.test.mjs` 已接入 `test:thread-chat:artifact-references`：以实际 AI SDK `convertToModelMessages` 输出计数，验证自身源工具、继承历史与跨轮重复引用的正文仅有一份。
- 覆盖无源历史、源被移除、临时/不完整工具被 SDK 忽略、正文/标题/来源/回传 ID 不匹配、同名不同 ID、dynamic-tool、前向引用及持久 Parts 不变。
- 固定引用标记跨消息完全一致；追加追问前后，对最终模型消息前缀执行 JSON 字节一致性断言，且同输入重复编译一致。
- 这验证请求内容与缓存前缀稳定性，不代表真实提供商 token 账单或缓存命中率实测；已有数据库和模型联调仍待验。
