...
系统 SHALL 仅允许已登录的资源所有者为自己的 Project 或 Document 创建分享。系统 MUST 从服务端持久化数据构建快照，并在同一一致读取事务内保存内容、初始布局、token 和生命周期字段。系统 MUST NOT 接受客户端正文、所有者身份或时间作为权威值，也 MUST NOT 自动把任何旧资源设为公开。
...
#### Scenario: Create a Document snapshot

- **WHEN** 所有者提交自己的已存在 Document
- **THEN** 系统 pin 创建时的 currentRevisionId 对应版本（revisionId/revisionNumber/artifactId/标题/正文）并返回独立匿名链接

#### Scenario: Reject another owner's resource

- **WHEN** 用户提交另一个用户的 Project 或 Document ID，或伪造跨 Project 关联
- **THEN** 系统拒绝创建，不返回他人正文、标题或 token，不保存分享

#### Scenario: Reject client supplied private fields

- **WHEN** 创建请求附带正文、ownerId、token、expiresAt、instructions 或未支持的分享类型
- **THEN** 系统按严格请求契约拒绝该输入，不把这些字段持久化为快照

#### Scenario: Source changes during creation

- **WHEN** 生成快照期间另一个请求新增 Thread、替换 Message 或提交文档新版本
- **THEN** 成功的快照来自同一数据库可见时点，不拼接不同轮读取的状态；失败不产生部分可访问快照
...
- **WHEN** 创建分享后原 Project 新增内容、编辑、重命名、重新生成、文档提交新版本或改变工作台布局
...
### Requirement: Project snapshot preserves readable branches and documents

Project 快照 SHALL 覆盖全部 Thread，而不仅是当前打开的列。系统 MUST 保存当前可读消息，以及分支冻结上下文、分叉来源和 Artifact/Document 来源所需的最小历史集合，保持快照内关联闭合。快照 SHALL 纳入项目全部已完成 Artifact 正文与 Document 目录（pin 分享时当前版本）。系统 MUST NOT 顺带暴露无关历史版本、文档历史版本或从私有源接口补全缺失内容。
...
- **WHEN** Project 有不被任何分支或纳入 Artifact/Document 阅读关系引用的已替换消息
- **THEN** 这些消息不进入公开快照

#### Scenario: Cross Project or missing reference

- **WHEN** 构建快照发现必要消息/Artifact/Document 引用不存在或归属其他 Project
- **THEN** 创建失败且不保存公开链接，不去其他 Project 读取正文或生成断裂快照

#### Scenario: Document shows share-time revision only

- **WHEN** 分享后文档提交了新版本
- **THEN** 快照内文档仍显示创建时 pin 的版本，不提供版本切换入口；消息中指向历史 artifact 的引用仍可解析

### Requirement: Generation remains static in a snapshot

系统 SHALL 将分享时的生成状态表示为静态阅读状态，不等待、启动、恢复或订阅模型。系统 MUST NOT 发布瞬态工具输入或内部错误详情。
...
- **THEN** 快照可呈现其持久化可见正文和简洁状态，不包含内部 error code/message，也没有 Retry 或 Stop 入口

### Requirement: Standalone Document share has an isolated scope

系统 SHALL 为已存在的 Document 创建独立分享，冻结其创建时当前版本。该快照 MUST 仅含文档标识、pin 的版本信息、公开标题、正文和产物时间，不包含来源对话正文、版本历史、私有来源导航或来源 Project/Thread 元信息。

#### Scenario: Read one Document anonymously

- **WHEN** 读者打开有效 Document 分享链接
- **THEN** 能阅读该文档该版本，页面与接口均不返回来源 Project/Thread/Message、其他版本或其他文档

#### Scenario: Source Project also has a share

- **WHEN** 同一文档的来源 Project 另有分享链接
- **THEN** Document 链接仍不自动暴露该 Project 链接或获得其访问权限
...
- **THEN** 返回统一的分享不可用页面/响应，不包含资源标题、正文、owner 信息或失效原因区分

#### Scenario: A reader probes private endpoints

- **WHEN** 读者用分享内 ID 或 token 请求私有 Project、Message、Artifact、Document、附件或生成 API
- **THEN** 原登录和所有权门禁仍生效，分享不授予新访问权或写权限

### Requirement: Public output excludes private fields before storage

系统 MUST 使用显式公开白名单在持久化前构造快照，并在响应时只序列化公开契约。系统 MUST NOT 保存或下发 Project target/instructions/contractVersion、Message feedback/error/providerUsage/documentContextUsed、Thread modelId 与标题生成内部状态、Files/附件元数据与字节、签名地址、账号信息、内部配置；未知 message part 或 metadata MUST 默认排除。仅删除 UI 入口或把敏感字段置为不可见不满足本要求。

#### Scenario: Private fields contain sentinel values

- **WHEN** 源对象的 instructions、Files、providerUsage、内部 error、未知 part 含不同的测试秘密哨兵
- **THEN** 持久化快照、匿名 JSON、HTML/RSC、DOM 和 metadata 中均不存在这些哨兵

#### Scenario: Read approved message parts

- **WHEN** Message 包含 text、quote、reasoning、研究轨迹、来源信息和 tool parts
- **THEN** 正文与引用被逐字段处理；其他内容仅返回已明确审查的阅读字段，不透传 provider metadata、工具原始输入、readUrl 抓取全文或文档工具 output 正文
...
系统 MUST 在持久化前清洗进入公开阅读内容的私有附件路径（`/api/attachments/*` 相对与绝对形式）、对象存储/签名地址和不安全协议，覆盖正文、quote、研究摘要与来源链接。公开页面 MUST NOT 下载、代签、内联附件或执行 HTML/脚本，也 MUST NOT 自动加载外部图片。安全外部资料链接可供读者主动打开，外部页面内容不属于冻结范围。

#### Scenario: Private links appear in multiple Markdown forms

- **WHEN** 私有附件 URL 以相对/绝对地址、引用式链接、自动链接、图片或纯文本形式出现
...
Project 分享 SHALL 在创建确认时从 `ConversationStore.workspace` 与 overlay 状态捕获视图模式、打开的 Thread 及顺序、折叠/列宽/焦点、画布 pins/viewport、面板尺寸、树展开节点和打开的 Artifact。系统 MUST 校验所有 ID 属于快照、数值有限有界；失效引用规范化为安全默认值。分享链接不依赖作者的 localStorage 或 URL 参数即可恢复初始布局。
...
- **THEN** 服务端规范化失效引用为安全初始值或拒绝非法数值，不读取快照外对象
...
匿名与已登录读者在分享页 SHALL 能切换列/画布、打开/关闭/折叠 Thread、调整阅读布局、定位引用、阅读 Artifact/Document 和复制文本。分享页 SHALL 复用与私有工作台相同的组件树，并以统一的只读状态位把写控件（Composer、消息操作、Fork/Retry/Edit/Stop、反馈、重命名、删除、上传、批注、文档更新入口）呈现为禁用态或省略。系统 MUST NOT 从分享页发起任何业务写请求或装配模型运行时；服务端无需为分享引入新的鉴权类型，到达的写请求按现有登录/所有权规则拒绝。
...
- **WHEN** 所有者从 Project 顶栏或 Document/Artifact 详情打开弹窗
- **THEN** 看到四个有效期选项、默认无限及上述提示，创建前能理解分享范围

### Requirement: Oversized or incomplete snapshots fail without publication

系统 SHALL 对创建输入、布局规模和快照总尺寸执行明确的有界校验（上限取宽松值，集中定义于 constants）。超过限制或无法满足必要关系完整性时 MUST 返回明确失败，不生成公开链接，也 MUST NOT 静默截断后声称已分享整个 Project。
