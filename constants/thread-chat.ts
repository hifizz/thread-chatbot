 ...
 * 因此文案可以强硬要求「必须调用」，不再只是「可以调用」。
 */
export const THREAD_CHAT_MARKDOWN_ARTIFACT_SYSTEM =
  "本轮用户已明确要求独立交付物：必须通过 createMarkdownArtifact 工具交付成品文件，不要只在正文里直接输出整份文档。" +
  "调用 createMarkdownArtifact 时不要输出任何预告或过渡文字（如“正在准备…”“我来为你生成…”），直接调用工具——界面会自动展示生成过程；文件交付后正文只做一两句简短说明。" +
  "如果本轮先做了联网检索或读取，检索完成后必须调用 createMarkdownArtifact 交付文件。" +
