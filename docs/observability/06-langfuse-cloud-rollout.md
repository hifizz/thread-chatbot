...
3. 将 `AI_OBSERVABILITY_ENVIRONMENT=staging`、`AI_OBSERVABILITY_RELEASE=<git-sha-or-image-tag>`、`AI_TELEMETRY_RECORD_CONTENT=true`、`AI_DEVTOOLS_ENABLED=false` 固定到 staging。
...
先开启 `AI_TELEMETRY_ENABLED=true`、`AI_LANGFUSE_ENABLED=true` 和 `AI_TELEMETRY_RECORD_CONTENT=true`。逐一运行并在 Langfuse 核对：

| 场景          | 必须看到                                             | 不得看到                 |
| ------------- | ---------------------------------------------------- | ------------------------ |
| 普通回答      | 单一 Message Trace、model step、usage、完整 input/output、completed | 凭据字段原值             |
| research      | route、plan、answer 父子关系与完整内容                         | 凭据字段原值             |
| Search/Fetch  | 每个 provider attempt、outcome、完整 query/URL/结果、usage unit | 凭据字段原值             |
| Artifact/tool | 同一根 Trace 下的 tool step、参数、结果和 reasoning             | 凭据字段原值             |
| Stop/失败     | stopped/failed、错误类别、可用的原始 provider 信息、数据库终态 | 凭据字段原值             |
...
- 日志和 Langfuse 出口不会暴露命名为 authorization、cookie、apiKey、secret、password、token 或 credentials 的字段值。
...
2. 对内部/低流量请求开启完整内容记录，记录 release 与起止时间。
...
5. 生产是否记录内容只由 `AI_TELEMETRY_RECORD_CONTENT` 决定；需要紧急停止内容采集时设为 `false`，需要停止全部遥测时设 `AI_TELEMETRY_ENABLED=false`。
