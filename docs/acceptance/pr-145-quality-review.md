...
- 预览和当前目录共享来源状态 selector。当前来源从 generating 变为 completed/failed 时取目录状态；历史版本、普通产物和固定缓存保持原样。
- 两条查询共用 DocumentListItemDTO 映射；原生测试逐文档比较查找结果与目录结果完全一致，同时保留单 SQL 次数、并发快照和原有锁竞争检查。
- 本轮 `pnpm typecheck`、相关 ESLint、文档专项、客户端 store、原生独立 PostgreSQL 44 项与差异检查通过。未重跑浏览器/模型验收，没有数据库结构或迁移修改。
