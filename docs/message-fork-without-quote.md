...
复用现有分叉接口。`sourceMessageId` 必填，`target` 可省略或为 `null`；提供 `target` 时按 `type` 区分 `message`/`artifact` 来源并携带选区锚点，引用文本统一由 `target.anchor.quote.exact` 派生。省略 `target` 即从整条已完成的 AI 回复直接分叉。持久化时，无引用分支的 `forkAnchor`、`anchorText` 和 `forkArtifactId` 均为 `null`。
...
`threads_root_or_fork_shape` 允许 `forkAnchor`、`anchorText`、`forkArtifactId` 三个引用字段同时为空，继续保留来源消息、父分支、编号和历史非空的约束。

迁移 `0011_motionless_wong.sql`：SQL 只替换 CHECK 约束，不改写现有记录。原 `0008_motionless_wong.sql` 在本地 `develop` 集成分支生成；rebase 到 main 后因 main 已占用 0008–0010（含 `fork_artifact_id` 列），重编号为 0011 并同步补齐 `fork_artifact_id` 的成对约束，snapshot/journal 一并更新。远端没有 `develop` 分支；此处没有向远端创建或发布 `develop`。

已用 PGlite 0.5.8（PostgreSQL 18.3 WASM）验证从旧迁移 0004 中的原始 threads 建表 SQL 升级：旧约束拒绝无选区分叉，新约束接受；原有主线和划选分支数据不变；半套锚点、缺来源、空继承历史和非法主线继续被拒绝。验证范围是独立 threads 表及 CHECK 约束，没有执行完整迁移链、外键或真实应用事务。
