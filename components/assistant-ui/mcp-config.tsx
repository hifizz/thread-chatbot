...
const CustomServersSection: FC = () => {
  ...
  return (
    <section className="aui-mcp-custom-servers flex flex-col gap-2">
      ...
      {/* react-mcp 的 primitive 是 Radix 风格：组合用 asChild（Base UI 的 render prop 不被支持） */}
      {!showForm && (
  ...
  return (
    <McpAddFormPrimitive.Root onSubmitted={onClose} onCancel={onClose}>
      <div className="aui-mcp-add-form flex flex-col gap-3 rounded-lg border p-3">
        ...
        <FormRow label="Auth">
          ...
          <div
            className={cn(
              ...
              "empty:hidden [&_input]:flex [&_input]:h-9 [&_input]:w-full [&_input]:rounded-md [&_input]:border [&_input]:border-input [&_input]:bg-transparent [&_input]:px-3 [&_input]:py-1 [&_input]:text-sm [&_input]:shadow-xs [&_input]:transition-[color,box-shadow] [&_input]:outline-none",
        ...
        <McpAddFormPrimitive.Error className="text-xs text-destructive" />
