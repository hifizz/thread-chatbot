...
export function ModelCatalogManager({ initial }: { initial: Catalog }) {
  ...
  return <>
    <div className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h1 className="text-2xl font-semibold tracking-tight">模型管理</h1><p className="text-sm text-muted-foreground">管理可用模型、能力与生成默认值。保存后，新请求使用最新配置。</p></div><Button onClick={() => setEditor(newModel(catalog.models.length))} disabled={busy}><Plus />新增模型</Button></div>
