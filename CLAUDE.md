...
- Theming via `next-themes` through `components/theme-provider.tsx`, wired up in `app/layout.tsx`. Defaults to system preference; manual selections persist in localStorage. The thread-chat appearance menu offers system/light/dark on desktop and mobile. Globally, `D` toggles light/dark and `Shift+D` restores system preference outside text inputs.
...
## Thread Chat 持久化文档工具结果

`messages.parts` 是流式 checkpoint，`messages.documentToolParts` 保存独立持久化的文档工具结果。展示消息或构建模型上下文时，必须使用 `restoreDocumentToolParts` 合并后的内容；复用 `toMessageDTO`、`toConversationMessage` 或上下文编译入口，不直接把原始 `messages.parts` 当作完整消息。仅检查用户输入、消息角色等操作可以读取原始字段。

恢复时，已有相同 `toolCallId` 的结果原位替换；缺失调用位置的结果按持久化顺序追加。这是保留已保存事实的兜底，不保证还原其相对于其他文本或工具的原始顺序。当前没有独立事件排序协议，不得将追加顺序解释为真实执行顺序。
