...
assert.equal(forkThreadCommandSchema.safeParse({ ...bareCommand, target: null }).success, true)
...
assert.equal(forkThreadCommandSchema.safeParse({ ...bareCommand, target: { type: "message", anchor } }).success, true)
assert.equal(forkThreadCommandSchema.safeParse({ ...bareCommand, target: { type: "artifact", artifactId: id(), anchor } }).success, true)
for (const fields of [
  { target: { type: "message" } },
  { target: { type: "artifact", anchor } },
  { target: { type: "message", anchor: { quote: { exact: "", prefix: "", suffix: "" } } } },
  { anchor, anchorText: "选区" },
]) {
...
assert.equal(buildThreadChatSystem({}), buildThreadChatSystem())
...
await assert.rejects(() => commands.forkThread({
  parentThreadId: parent.id, sourceMessageId: source.id, modelId: parent.modelId,
  target: { type: "message", anchor: { quote: { exact: "", prefix: "", suffix: "" } } },
}))
...
// 新版 parts 契约下，带首问的直接分叉也不能构造空来源 Quote。
const boundaryFailure = new Error("受控边界失败")
let firstTurnChecked = false
const firstTurnCommands = createConversationCommands({
  store, networkAttempts: 1,
  client: {
    ...mock.client,
    async forkThread(_threadId, command) {
      forkThreadCommandSchema.parse(command)
      assert.deepEqual(command.firstTurn.parts, [{ type: "text", text: "继续解释" }])
      assert.deepEqual(store.getState().messagesById[command.firstTurn.userMessageId].parts, command.firstTurn.parts)
      firstTurnChecked = true
      throw boundaryFailure
    },
  },
})
await assert.rejects(() => firstTurnCommands.forkThread({
  parentThreadId: parent.id, sourceMessageId: source.id, modelId: parent.modelId,
  firstTurn: { parts: [{ type: "text", text: "继续解释" }] },
}), (error) => error === boundaryFailure)
assert.equal(firstTurnChecked, true)
assert.equal(Object.keys(store.getState().messagesById).length, before)

firstTurnCommands.dispose()
