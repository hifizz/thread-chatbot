...
try {
  ...
  const targetIndex = journal.entries.findIndex((entry) => entry.tag === "0011_motionless_wong")
  ...
  await commands.forkThread(userId, start.rootThreadId, { ...fork, target: { type: "message", anchor: { quote: { exact: "迁移验收原文", prefix: "", suffix: "" } } } })
