...
import { MODEL_COST, priceFromCost, usdToMicros } from "../../constants/pricing.ts"
...
assert.deepEqual(models.map((model) => model.upstreamModel), expectedSlugs)
...
assert.notEqual(
  ...
  models.find((model) => model.id === "openrouter-deepseek-v4-flash-0731")?.provider
)
...
const step = (cost) => ({ providerMetadata: { openrouter: { usage: { cost } } } })
...
for (const steps of [[], [step(0.1), {}], [step(-1)], [step("1")], [step(NaN)], [step(Infinity)]]) {
