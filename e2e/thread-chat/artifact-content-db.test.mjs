...
async function rejected(command, user = userId) {
  await assert.rejects(() => commands.sendMessage(user, source.rootThreadId, command, fixtureModelCatalog))
...
try {
  ...
  await commands.startProject(userId, source, fixtureModelCatalog)
  ...
  await commands.startProject(userId, foreign, fixtureModelCatalog)
  ...
  await commands.sendMessage(userId, source.rootThreadId, boundary, fixtureModelCatalog)
  ...
  const saved = (await commands.sendMessage(userId, source.rootThreadId, own, fixtureModelCatalog)).result
  assert.deepEqual(saved.userMessage.parts.map((part) => part.type), ["text", "data-artifact-reference", "text", "data-artifact-reference"])
  const replay = (await commands.sendMessage(userId, source.rootThreadId, own, fixtureModelCatalog)).result
  ...
  await db.update(schema.messages).set({ status: "stopped" }).where(eq(schema.messages.id, source.assistantMessageId))
  const edited = (await commands.editLatestTurn(userId, own.userMessageId, { ...turn(), parts: own.parts }, fixtureModelCatalog)).result.generation
  ...
  for (const quotes of [[legacyInput, legacyInput], [{ type: "quote", quote: { text: "改写" } }]]) {
    await assert.rejects(() => commands.editLatestTurn(userId, edited.userMessage.id, { ...turn(), parts: [...quotes, { type: "text", text: "编辑" }] }, fixtureModelCatalog))
  }
  const kept = (await commands.editLatestTurn(userId, edited.userMessage.id, { ...turn(), parts: [legacyInput, { type: "text", text: "编辑后" }] }, fixtureModelCatalog)).result.generation
  assert.deepEqual(kept.userMessage.parts[0], legacy)
  await settle(kept.assistantMessage.id)
  const removed = (await commands.editLatestTurn(userId, kept.userMessage.id, turn("删除引用"), fixtureModelCatalog)).result.generation
  ...
  await assert.rejects(() => commands.startProject(userId, startBad, fixtureModelCatalog))
